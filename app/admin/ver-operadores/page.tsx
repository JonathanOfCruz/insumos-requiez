...
// Definición de la interfaz para los datos del operador
interface IOperator {
    _id: string;
...
export default function VerOperadoresPage() {
    ...
    // Elimina el registro mediante DELETE con confirmación
    const handleDelete = async (operator: IOperator) => {
        const confirmDelete = window.confirm(`¿Estás seguro de que deseas eliminar al operador ${operator.name}?`);
    ...
    return (
        <main className="min-h-screen bg-[#f4f7fa] p-6 lg:p-10">
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                <div className="w-10 h-10 bg-[#0095ff] rounded-full flex items-center justify-center text-white font-bold text-xs shadow">{initials}</div>
            </div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 overflow-x-auto relative">
                ...
                <div className="flex justify-end mb-6">
                    <div className="flex items-center gap-2 border rounded-full px-3 py-1.5 text-xs text-gray-300">
                        <Icon icon="solar:magnifer-linear" />
                        <input type="text" placeholder="Buscar..." className="outline-none bg-transparent" readOnly />
                    </div>
                </div>
                                    ...
                                    <td className="px-4 py-4 text-gray-600">
                                        {isEditing ? (
                                            <input
                                                ...
                                                className="w-full text-gray-600 border-b border-blue-400 outline-none p-1 bg-blue-50"
                                            />
                                        ) : (
                                            operator.name
                                        )}
                                    ...
                                    <td className="px-4 py-4 text-center text-gray-600">
                                        {isEditing ? (
                                            <select
                                                ...
                                                className="border-b border-blue-400 outline-none p-1 bg-blue-50 text-xs font-bold uppercase"
                                            >
                                    ...
                                    <td className="px-4 py-4 text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            {isEditing ? (
                                                <>
                                                    <button
                                                        onClick={() => handleSaveEdit(operator._id)}
                                                        className="text-green-500 bg-green-50 p-2 rounded-lg hover:bg-green-100"
                                                    >
                                                        <Icon icon={savingId === operator._id ? "solar:restart-linear" : "solar:diskette-bold"} className={savingId === operator._id ? 'animate-spin' : ''} />
                                                    </button>
                                                    <button
                                                        onClick={() => { setEditingId(null); setTempEditData(null); }}
                                                ...
                                                <>
                                                    <button
                                                        ...
                                                        className="text-amber-500 bg-amber-50 p-1.5 rounded hover:bg-amber-100"
                                                    >
